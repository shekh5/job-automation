...
- **CRITICAL**: You must ONLY use the `bash` tool to run either the `ats_apply.py` or `stealth_apply.py` script as documented in your `TOOLS.md` file.
...
- **HUMAN APPROVAL WORKFLOW**: The `stealth_apply.py` script will stop right before final submission, leaving the browser open, and will output the `SESSION_ID` and a `SCREENSHOT_PATH`. When it does this:
